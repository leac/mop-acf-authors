	...
	class Mop_Acf_Authors_Public {
		...
		public function show_post_author_info( $author_fields, $post_id = null ){
			...
			foreach ( $this->acf_field_ids as $acf_field_id ) {
				...
				if ( ! empty( $this->authors_collection[$acf_field_id]['authorname'] ) ) { // the check for ['authorname'] is ugly. should be done inside set_authors_array
...
?>
		...
		/**
		 *
		 * @param int $author_array_ind - index of $this->authors_collection to isert info to
		...
		function show_author_name( $author_group_id, $post_id = null ){
			...
			if ( ! empty( $this->authors_collection[count( $this->authors_collection )]['authorname'] ) ) {
				$ret.= '<a href="' . esc_url( add_query_arg( 'author', urlencode( $this->authors_collection[count( $this->authors_collection )]['authorname'] ), get_page_link( $this->authors_page_id ) ) ) . '">' . $this->authors_collection[count( $this->authors_collection )]['authorname'] . '</a>';
			}
		...
		function show_author_image( $author_group_id, $post_id = null ){
			...
			if ( ! empty( $this->authors_collection[count( $this->authors_collection )]['authorimage'] ) ) {
				...
				$ret = wp_get_attachment_image( $this->authors_collection[count( $this->authors_collection )]['authorimage']['id'], 'scientific-2016-fast_science_size' );
			}
			else {
		...
		function show_author_description( $author_group_id, $post_id = null ){
			...
			if ( ! empty( $this->authors_collection[count( $this->authors_collection )]['authordescription'] ) ) {
				$ret.= '<span>' . $this->authors_collection[count( $this->authors_collection )]['authordescription'] . '</span>';
			}
			return $ret;
		}
